...
function App() {
  ...
  useEffect(() => {
    ...
    // ════════════════════════════════════════════════════════════
    supabase.auth.getSession()  // Demande à Supabase : "Quelqu'un est connecté ?"
      .then(({ data: { session } }) => {  // Récupère la réponse
    ...
    } = supabase.auth.onAuthStateChange(  // S'abonne aux événements
      (_event, session) => {  // Callback appelé à chaque changement
        // _event peut être :
