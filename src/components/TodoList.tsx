import { Check, Circle, LogOut, Plus, Trash2 } from "lucide-react";
...
export default function TodoList() {
	...
	const addTodo = async (e: React.FormEvent) => {
		e.preventDefault();
		// Cette condition vérifie si le texte saisi pour la nouvelle tâche est vide ou juste constitué d'espaces :
		// - newTodo.trim() enlève les espaces en début et fin de chaîne.
		// - Si, après ce nettoyage, il ne reste rien ("falsey"), la fonction quitte immédiatement (return).
		// Cela évite d'ajouter des tâches vides à la liste.
	...
	const handleSignOut = async () => {
		// Cette ligne appelle la méthode signOut() du module d'authentification Supabase pour déconnecter l'utilisateur courant de l'application (suppression du token côté client).
		await supabase.auth.signOut();
	};

	// On calcule le nombre de tâches "actives" (non terminées)
	const activeTodos = todos.filter((t) => !t.completed).length;
	// On calcule le nombre de tâches "terminées"
							...
							{todos.length === 0 ? (
								...
								<div className="text-center py-12 text-slate-400">
									...
									<p className="text-sm mt-2">
										Commencez par en ajouter une ci-dessus
									</p>
								</div>
							) : (
								todos.map((todo) => (
